...
How it works:
  1. The public Hugging Face Space "AIRI-Institute/HairFastGAN" already
     runs the model on a shared GPU, for free, under an MIT license.
  ...
  3. This proxy itself needs no GPU — it can run on a free CPU-only
     host (e.g. a free Hugging Face "Docker" Space, or Render's free
     tier). See README.md for exact deploy steps.

IMPORTANT — one thing I could not verify from this environment:
  I don't have live network access to actually call the Space and
  confirm the request succeeds end-to-end. The parameter names below
  (face / shape / color / blending / poisson_iters / poisson_erosion,
  api_name="/swap_hair") come directly from the model authors' own
  published usage example. Hugging Face Spaces can change their code
  over time, so if this returns an error after you deploy it, run:

      from gradio_client import Client
      Client("AIRI-Institute/HairFastGAN").view_api()
...
# The underlying open-source model, hosted for free by its authors.
# Swap this for your own duplicated Space (see README) if you outgrow
# the shared public queue.
...
async def generate_preview(
    ...
    blending: str = Form("Article"),
    poisson_iters: int = Form(0),
    poisson_erosion: int = Form(15),
):
    """
    ...
    try:
        ...
        try:
            result = client.predict(
                face=gr_file(face_path),
                shape=gr_file(shape_path),
                color=gr_file(color_path),
                blending=blending,
            ...
            raise HTTPException(
                ...
                detail=(
                    ...
                    "Client('AIRI-Institute/HairFastGAN').view_api() to see "
